...
// app.use(noCache);
        ...
        if (this.xpl) {
            app.get(/^\/xplCommand\/(.*)$/, nocache, (request, response) => this._xplCommand(request, response));
            app.get(/^\/tunnelCommands(.*)$/, nocache, (request, response) => this._proxyCommands(request, response));
        }
        ...
        if (this.configuration.socketIO) {
            ...
            io.on('connection', function (socket) {
                ...
                function xplMessage(message) {
                    ...
                    socket.emit('xpl-message', message);
                }

                socket.on('disconnect', function () {
                    console.log('WS user disconnected uuid=', clientUUID);

                    this.removeListener(xplMessage);
                });
