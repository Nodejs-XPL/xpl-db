...
//  app.use(noCache);
    ...
    if (this.xpl) {
      app.get(/^\/xplCommand\/(.*)$/, this.xplCommand.bind(this));
    }

    app.post('/last', this.postLast.bind(this));
    app.post('/history', this.postHistory.bind(this));
    app.post('/minMaxAvgSum', this.postMinMaxAvgSum.bind(this));
  ...
  _xplCommand(request, response) {
    ...
    var commmand = query.c || query.cmd || query.command
    ...
    debug("xplCommand", "key=", key,"command=",command,"device=",device,"target=",target,"bodyName=",bodyName,"current=",current);
    ...
    var source="http."+req.connection.remoteAddress;
    ...
    xpl.sendXplCmnd(params, bodyName, target, source, (error) => {
