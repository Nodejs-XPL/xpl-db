        ...
        if (this.configuration.socketIO) {
            ...
            io.on('connection', (socket) => {
                ...
                function xplMessage(message) {
                    ...
                    if (filter) {
                        if (filter.type === 'devices-only' && filter.devices) {
                            ...
                            let device = message.body.device;
                            if (message.body.type) {
                                device += '/' + message.body.type;
                            }

                            if (!filter.devices[device]) {
                                console.log('Ignore device=', device, 'for client', clientUUID);
                                return;
                            }
                        }
                    }
                ...
                socket.on('xpl-command', (message) => {
                    ...
                    const command = message.c || message.cmd || message.command;
                    ...
                    if (!device || !command) {
                        socket.emit('xpl-error', {id, code: 400, message: "Device or command not found"});
                        return;
                    }
                    ...
                    this.xpl.sendXplCmnd(params, bodyName, target, source, (error) => {
                        ...
                        if (error) {
                            ...
                            socket.emit('xpl-error', {id, code: 500, message: "Internal error"});
                            return;
                        }

                        socket.emit('xpl-result', {id, code: 200, message: "Command sent"});
