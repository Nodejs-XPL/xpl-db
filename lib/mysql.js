...
Mysql.prototype._save = function(message, connection, callback) {
  ...
  var self = this;
  this._getTypeById(deviceName, this.deviceIds, "devices", connection,
      function(error, deviceId) {
        if (error) {
          return callback(error);
        }
        ...
        self._getTypeById(sourceName, self.sourceIds, "sources", connection,
            function(error, sourceId) {
              ...
              if (SOURCE_SUPPORT) {
                debug("Source=", sourceName, "=>", sourceId);
              }

              if (!units) {
                return self._insertCurrent(deviceId, sourceId, undefined,
                    current, date, connection, callback);
              }

              self._getTypeById(units, self.unitIds, "units", connection,
                  function(error, unitId) {
                    ...
                    self._insertCurrent(deviceId, sourceId, unitId, current,
                        date, connection, callback);
                  });
            });
      });
