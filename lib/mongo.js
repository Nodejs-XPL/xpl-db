...
class Mongo extends Db {
	...
	connect(callback) {
		...
		MongoClient.connect(url, (error, db) => {
			...
			messages.createIndex({date: 1}, {name: 'byDate'}, (error, result)=> {
				...
				messages.createIndex({device: 1, date: 1}, {name: 'byDeviceDate', sparse: true}, (error, result)=> {
	...
	_createCursor(deviceKey, options, order, limit, projectCurrent) {
		...
		cursor = cursor.sort({'date': order}).hint( "byDeviceDate" );
		...
		if (debug.enabled) {
		  debug("_createCursor", "Explain=", cursor.explain());
		}
